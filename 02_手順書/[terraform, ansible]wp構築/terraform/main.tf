...
# Security Groups
#######################################
# 共通: SSH(22)は自分のIP(local.my_ip_cidr_effective)のみ許可 / outbound all allow
...
resource "aws_security_group" "web" {
  ...
  ingress {
    ...
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = [local.my_ip_cidr_effective]
  }
...
resource "null_resource" "update_ansible_files" {
  depends_on = [
    aws_instance.web,
    aws_instance.ap,
    aws_instance.db,
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      AP_PUBLIC_IP         = aws_instance.ap.public_ip
      DB_PUBLIC_IP         = aws_instance.db.public_ip
      INNER_DNS_PUBLIC_IP  = aws_instance.inner_dns.public_ip
      WEB_PRIVATE_IP       = aws_instance.web.private_ip
      AP_PRIVATE_IP        = aws_instance.ap.private_ip
      DB_PRIVATE_IP        = aws_instance.db.private_ip
      INNER_DNS_PRIVATE_IP = aws_instance.inner_dns.private_ip
    }

    command     = "chmod +x ./scripts/update_ansible_files.sh && ./scripts/update_ansible_files.sh"
  }
}

# 2) var.run_after_apply=true のときだけ Ansible を実行
resource "null_resource" "run_all" {
  count      = var.run_after_apply ? 1 : 0
  depends_on = [
    null_resource.update_ansible_files
  ]

  # update_ansible_files が走り直したら ansible も走り直す
  triggers = {
    ansible_run_key = join(",", [
      aws_instance.web.public_ip,
      aws_instance.ap.public_ip,
      aws_instance.db.public_ip,
      aws_instance.inner_dns.public_ip,
      aws_instance.web.private_ip,
      aws_instance.ap.private_ip,
      aws_instance.db.private_ip,
      aws_instance.inner_dns.private_ip
    ])
  }
  ...
  provisioner "local-exec" {
    working_dir = path.module
    interpreter = ["/bin/bash", "-lc"]
    #command = "chmod +x ./scripts/wait_for_ssh.sh && ./scripts/wait_for_ssh.sh ./ansible/inventory/hosts.ini && ansible-playbook -i ./ansible/inventory/hosts.ini ./ansible/site.yml"
    command = <<EOT
    chmod +x ./scripts/wait_for_ssh.sh && \
    ./scripts/wait_for_ssh.sh ./ansible/inventory/hosts.ini && \
    ...
    echo "========================================" && \
    echo "WordPress URL:" && \
    echo "  http://${aws_instance.web.public_ip}/" && \
    echo "Admin:" && \
    echo "  http://${aws_instance.web.public_ip}/wp-admin/" && \
    echo "========================================"
    EOT
  }
}
