...
# ログ設定
#######################################
# 実行中のスクリプト名（フルパス） → ファイル名だけ取得 → .sh を除去
...
read -p "EC2 のパブリック IP（例: 35.xxx.xxx.xxx）: " EC2_PUBLIC_IP
while [[ -z "${EC2_PUBLIC_IP}" ]]; do
  read -p "EC2 のパブリック IP（空は不可）: " EC2_PUBLIC_IP
  echo
done
