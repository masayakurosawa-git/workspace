...
▼ 作成
```bash
#create ltm node <node名> address <IPaddress>
...
▼ 作成
```bash
#create ltm pool <pool名> members add { node名1:80 node名2:80 node名3:80 node名4:80 node名5:80 }
...
▼ 作成
```bash
#create ltm virtual <VirtualServer名> destination <VirtualServerに割り当てるIP>:http ip-protocol tcp
