## FW接続
```bash
# ローカル → 踏み台
ssh -i ~/.ssh/entrycl_202601.pem ec2-user@16.144.198.187
# 踏み台 → FWサーバ
ssh -i ~/.ssh/entryCL_RSA_key.pem ec2-user@172.31.2.10
```

## インターフェース設定
1. 外部インターフェースの設定
    AWSの外部インターフェースのIPを設定
    ```bash
    set interfaces ge-0/0/0 unit 0 family inet address 172.31.0.10/24
    ```
</br>

2. 内部インターフェースの設定
    AWSの内部インターフェースのIPを設定
    ```bash
    set interfaces ge-0/0/1 unit 0 family inet address 172.31.1.10/24
    ```
</br>
</br>


## ポリシー設定
1. 信頼できるゾーンと信頼できないゾーンを作成
    ```bash
    # 信頼できないゾーン
    set security zones security-zone untrust interfaces ge-0/0/0.0
    
    # 信頼できるゾーン
    set security zones security-zone trust interfaces ge-0/0/1.0
    ```
</br>

2. ポリシーを設定(内部→外部)
    ```bash
    # 送信元アドレスがなんでもこのポリシーを適用
    set security policies from-zone trust to-zone untrust policy trust_to_untrust match source-address any

    # 送信先アドレスがなんでもこのポリシーを適用
    set security policies from-zone trust to-zone untrust policy trust_to_untrust match destination-address any
    
    # アプリケーションがなんでもこのポリシーを適用
    set security policies from-zone trust to-zone untrust policy trust_to_untrust match application any
    
    # ポリシー定義で全マッチングするとき、通信を許可する。
    set security policies from-zone trust to-zone untrust policy trust_to_untrust then permit

    # 設定適用
    commit
    ```
</br>

3. ポリシーを設定(外部→内部)
    ```bash
    # 送信先アドレスがなんでもこのポリシーを適用
    set security policies from-zone untrust to-zone trust policy untrust_to_trust match source-address any

    # 送信先アドレスがなんでもこのポリシーを適用
    set security policies from-zone untrust to-zone trust policy untrust_to_trust match destination-address any
    
    # アプリケーションがなんでもこのポリシーを適用
    set security policies from-zone untrust to-zone trust policy untrust_to_trust match application any
    
    # ポリシー定義で全マッチングするとき、通信を許可する。
    set security policies from-zone untrust to-zone trust policy untrust_to_trust then permit

    # 設定適用
    commit
    ```

</br>
</br>


## NAT設定
1. DestinationNATを作成
    ```bash
    # set security nat destination pool pool_A address <webサーバ/32>
    set security nat destination pool pool_teamD address 172.31.1.67/32
    ```
</br>

2. NATとインターフェースの紐付け
    ```bash
    # NATとインターフェースの紐付け
    #set security nat destination rule-set 1 from interface ge-0/0/0.0

    # ルール追加
    set security nat destination rule-set 1 rule 1D match destination-address 172.31.0.8/32

    # ルールにpoolを追加 
    set security nat destination rule-set 1 rule 1D then destination-nat pool pool_teamD

    # proxy_arpを設定
    #set security nat proxy_arp interface ge-0/0/0.0 address <EIPのパブリック>
    ```
</br>
</br>


## 削除
```bash
# rule「100」を削除
delete security nat destination rule-set 100

# rule「100」の「100A」を削除
delete security nat destination rule-set 100 rule 100A
```
